...
import { z } from 'zod';
import { activityFeedStatus } from './activity-feed-status';
 ...
 */
export const activityFeedResponse = z
  .object({
    resolvedAt: z.string().optional(),
    resolvedBy: z.number().optional(),
    status: activityFeedStatus.optional(),
 ...
 * Is equal to application shape if all property names match the api schema
 */
export const activityFeedRequest = z
  .object({ resolvedAt: z.string().nullish(), resolvedBy: z.number().nullish(), status: activityFeedStatus.nullish() })
  .transform((data) => ({
    resolvedAt: data['resolvedAt'],
    resolvedBy: data['resolvedBy'],
    status: data['status'],
