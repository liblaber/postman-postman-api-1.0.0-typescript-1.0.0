...
import { z } from 'zod';
import { historyResolution } from './history-resolution';
 ...
 * Is equal to application shape if all property names match the api schema
 */
export const historyRequest = z
  .object({ actor: z.number().nullish(), createdAt: z.string().nullish(), resolution: historyResolution.nullish() })
  .transform((data) => ({
    actor: data['actor'],
    createdAt: data['createdAt'],
    resolution: data['resolution'],
